...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    private void createNewRep(int hashTableSize) {
        ...
        //Creates a new bucket of data type Map2 for each array of hashTable
    ...
    public final void add(K key, V value) {
        ...
        //Finds the bucket that the key is in

        int k = mod(key.hashCode(), this.hashTable.length);
        //Adds the key value pair to the bucket fetched from before
        this.hashTable[k].add(key, value);
        //Increases the size of the table by 1
        this.size++;
    ...
    public final Pair<K, V> remove(K key) {
        ...
        //Finds the bucket that K key is in
        int k = mod(key.hashCode(), this.hashTable.length);
        //Decrease size of table by 1
        this.size--;
        //Returns the removed key value pair
        return this.hashTable[k].remove(key);
    }
    ...
    public final Pair<K, V> removeAny() {
        ...
        Map.Pair<K, V> p = null;
        boolean t = false;
        // for loop to remove an arbitrary key value pair if the bucket is not empty
        for (int i = 0; i < this.hashTable.length; i++) {
            if (this.hashTable[i].size() != 0 && !t) {
                t = true;
    ...
    public final V value(K key) {
        assert key != null : "Violation of: key is not null";
        assert this.hasKey(key) : "Violation of: key is in DOMAIN(this)";
        //Finds the bucket that k is in
        int k = mod(key.hashCode(), this.hashTable.length);
        //Returns value of key from bucket
        return this.hashTable[k].value(key);
    }
    ...
    public final boolean hasKey(K key) {
        ...
        //Finds the bucket that k is in
        int k = mod(key.hashCode(), this.hashTable.length);
        //Returns whether or not the bucket has the key
        return this.hashTable[k].hasKey(key);
    }
